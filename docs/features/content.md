...
- **Endpoint public de lecture ajouté dans une session suivante** (2026-09-16, suite) : la
  ...
  `collection-detail`/`nos-ateliers-liste` (9 des 14 pages Phase 1) le consomment. Les 5
  autres pages de la Phase 1 restent sur leurs littéraux codés en dur ; l'étape 4 de
  `docs/phases/phase-6-admin-cms.md` ("brancher les pages publiques") reste donc en cours,
  pas terminée — "migration progressive, pas un big-bang", une page à la fois.
...
- **Pages consommatrices** : `admin-gestion-contenu` (édition, endpoints `content/sections`).
  ...
  `collections-liste`, `collection-detail` et `nos-ateliers-liste` (lecture publique, endpoint
  `content/public/:page`) — 9 pages sur les 14 de la Phase 1, voir
  ...
  `docs/pages/nos-ateliers-liste.md` et l'écart ci-dessus. Les 5 autres pages publiques
  restent à migrer.
...
- [x] `home`/`a-propos`/`la-une`/`nos-creations-galerie`/`creation-detail`/`contact`/
      `collections-liste`/`collection-detail`/`nos-ateliers-liste` lisent réellement
      ...
      `useCollectionsContent`/`useCollectionDetailContent`/`useAteliersListeContent`), avec
