...
function buildRepository(quote: QuoteEntity | null): jest.Mocked<IQuoteRepository> {
  return {
    ...
    findById: jest.fn(),
    create: jest.fn(),
    update: jest.fn().mockResolvedValue(QuoteEntity.create(buildProps({ status: 'REJECTED' }))),
  };
}

describe('RejectQuoteUseCase', () => {
  it('throws NotFoundException for an unknown quoteNumber', async () => {
    const useCase = new RejectQuoteUseCase(buildCustomerRepository(), buildRepository(null));
  ...
  it('throws ForbiddenException when the quote belongs to another customer', async () => {
    ...
    const useCase = new RejectQuoteUseCase(buildCustomerRepository(sampleCustomer('customer-2', 'user-2')), repository);

    await expect(useCase.execute('ANG-DEV-2026-abc12345', 'user-2')).rejects.toThrow(ForbiddenException);
  });

  it.each(['DRAFT', 'ACCEPTED', 'REJECTED', 'EXPIRED'] as const)('rejects rejecting a %s quote', async (status) => {
    const repository = buildRepository(QuoteEntity.create(buildProps({ status })));
    const useCase = new RejectQuoteUseCase(buildCustomerRepository(), repository);

    await expect(useCase.execute('ANG-DEV-2026-abc12345', 'user-1')).rejects.toThrow(ConflictException);
    expect(repository.update).not.toHaveBeenCalled();
  });

  it.each(['SENT', 'VIEWED'] as const)('rejects a %s quote owned by the caller', async (status) => {
    const repository = buildRepository(QuoteEntity.create(buildProps({ status })));
    const useCase = new RejectQuoteUseCase(buildCustomerRepository(), repository);

    await useCase.execute('ANG-DEV-2026-abc12345', 'user-1');

    expect(repository.update).toHaveBeenCalledWith('quote-1', { status: 'REJECTED' });
  });
});
