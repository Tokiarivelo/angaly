...
describe('useJournalListeContent', () => {
  it('falls back to the hardcoded defaults when the CMS has no rows yet (default MSW handler)', async () => {
    const { result } = renderHook(() => useJournalListeContent(), { wrapper: withQueryClient() });

    await waitFor(() => expect(result.current.isLoading).toBe(false));

    expect(result.current.error).toBeNull();
    expect(result.current.data.header.title).toBe('Le Journal Angaly');
    expect(result.current.data.header.subtitle).toBe(
      "Mariage, mode, coulisses d'atelier et conseils d'entretien.",
    );
  });

  it('uses the CMS title/subtitle for the header section returned by the public endpoint', async () => {
    server.use(
      http.get(`${API_BASE_URL}/content/public/journal-liste`, () =>
        HttpResponse.json({
          success: true,
          data: [
            {
