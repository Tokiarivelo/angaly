...
describe('useGalleryContent', () => {
  it('falls back to the hardcoded defaults when the CMS has no rows yet (default MSW handler)', async () => {
    const { result } = renderHook(() => useGalleryContent(), { wrapper: withQueryClient() });

    await waitFor(() => expect(result.current.isLoading).toBe(false));

    expect(result.current.error).toBeNull();
    expect(result.current.data.header.title).toBe('Nos Créations');
  ...
  it('uses the CMS title/subtitle for the header section returned by the public endpoint', async () => {
    server.use(
      http.get(`${API_BASE_URL}/content/public/nos-creations-galerie`, () =>
        HttpResponse.json({
          success: true,
          data: [
            {
